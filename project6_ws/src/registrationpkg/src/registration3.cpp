#include"../include/registrationpkg/registration3.h"
...
Eigen::Matrix4f registration2::icp_nlRegistration(const pcl::PointCloud<PointTT>::Ptr& pc_scan, Eigen::Matrix4f transformation)
{
    ...
    icp_nl.setMaxCorrespondenceDistance(3);
    // 设置最大迭代次数（限制优化过程执行次数）
...
void cloudCallback(const sensor_msgs::PointCloud2::ConstPtr& msg)
{
    ...
    shared_data_mutex2.lock();
    pcl::fromROSMsg(*msg, *_cloud_current);
    shared_data_mutex2.unlock();
}
...
void registration2::calculate()
{
    ...
    std::cout<<"shared_data_mutex.lock();"<<std::endl;
    shared_data_mutex.lock();
    shared_data_mutex2.lock();
    Eigen::Matrix4f transform_ = _transform;
    cloud_current_->points.resize(_cloud_current->points.size());
    *cloud_current_ = *_cloud_current;
    shared_data_mutex.unlock();
    shared_data_mutex2.unlock();
    std::cout<<"shared_data_mutex2.unlock();"<<std::endl;
    if(cloud_current_ == nullptr)
    {
        std::cout<<"cloud_current_ == nullptr"<<std::endl;
        return;
    }
    else if(cloud_current_->points.size() <= 50)
    {
        std::cout<<"cloud_current_->points.size():" <<cloud_current_->points.size()<<std::endl;
        return;
    }
    std::cout<<"cloud_current_->points.size():" <<cloud_current_->points.size()<<std::endl;
    ROS_INFO("copy Time: %f", (ros::Time::now() - tic).toSec());
    Eigen::Matrix4f transform = icp_nlRegistration(cloud_current_, transform_);
    if(_transformation == false)
    {
        std::cout<<"fail to icp"<<std::endl;
        return;
